...
4. **Votes need login** (one switchable ±1 row per user/item, denormalized counts shown anonymously); reports allowed anonymous. The protection that ships **in the API** is a per-item cap — `ReportsOptions.MaxReportsPerItem` (20, from appsettings) enforced in `SubmitReportHandler`, returning 429 `REPORT_LIMIT_REACHED` at or over the cap — which bounds total table growth at catalog size × 20. Cloudflare rate rules (IP/ASN) remain the edge protection and stay owned by P6 Hardening.
...
- `versions`: Id, ItemType, ItemId, VersionNumber, SemanticVersion, FrozenManifestJson, ZipBlobPath, ZipSha256, SizeBytes, Status(Queued|Building|Published|Rejected|Failed), FailureReason; unique (ItemType, ItemId, VersionNumber). `ScanReportJson`: nullable JSON — `{ findings[{ruleId, category, severity, path, line, excerpt}], hookScriptCount, scannedFileCount, isTruncated }`, capped by `PublishingOptions.ScanReportJsonMaxLength`.
- `likes`: (UserId, ItemType, ItemId) PK, Value ±1
- `reports`: Id, ItemType, ItemId, ReporterUserId? (null exactly when the reporter was anonymous; `Guid.Empty` is normalized to null before it is stored), Reason(Malicious|Spam|Copyright|Other), Details (nullable, capped by `ReportsOptions.DetailsMaxLength`; required only when Reason is `Other`), Status(Open) — moderation states (`Reviewed`/`Dismissed`/`Actioned`) are deferred until an admin surface exists, so `Open` is the only state anything can reach today. Non-unique indexes on (ItemType, ItemId) — which serves the per-item cap count — and on ReporterUserId. No foreign key to `engineers`/`teams`, for the same reason `team_members` carries none: a reported item can be deleted without breaking its report history, and one column cannot point at two tables. There is no per-reporter uniqueness constraint.
...
- **Naming**: `e3a-{slug}` for engineers and `e3a-team-{slug}` for teams — the creator-typed slug is the plugin name; uniqueness enforced by the DB index, attribution via `author` (GitHub login). **An engineer slug may not begin `team-`**: that prefix is reserved for the team namespace, and rejecting it on the engineer side is what makes the two plugin namespaces disjoint. Without it an engineer slug `team-{x}` and a team slug `{x}` both produce `e3a-team-{x}`, so slug uniqueness scoped per table would not be enough.
...
Auth (anon): `GET /api/auth/github/login` → 302 to GitHub with a signed anti-CSRF `state` bound to the initiating browser by a `Secure`, `HttpOnly`, `SameSite=Lax` nonce cookie the callback compares and clears; `GET /api/auth/github/callback` → server-side code exchange → e3a JWT returned by a 302 to the web app with the token in the URL fragment (`#token=`), every failure returning `#error=<ERROR_CODE>` instead of a JSON error; `GET /api/auth/me` (auth) → the creator's profile. Catalog (anon): `GET /catalog?type&q&tag&sort&page&pageSize` (PageData), `GET /catalog/{slug}`, `GET /catalog/tags` (tags with counts), `GET /catalog/creators/{login}` (a creator's public profile: header + published engineers + published teams; unknown login → 404). Engineers: `GET /api/engineers/{id}` is anonymous (published to anyone; drafts owner-only: 401 anonymous / 403 non-owner); the anonymous published list lives on `/catalog` — while `GET /api/engineers/mine`, `GET /api/engineers/slug-availability?slug=` and all mutations are [auth/owner]: CRUD + upload + `POST {id}/publish → 202` + `POST {id}/unlist` + `POST {id}/relist`. Teams: mirror + members with pinned versions. `GET /publish/{versionId}/status` (poll, owner-only). Social: `POST /api/reports` (anon OK; signed-in callers are attributed) — 400 on an unknown item, 429 past the per-item cap; the web UI submits from engineer detail only, team reporting deferred until a team catalog endpoint exists. Worker: queue `publish-jobs`.

**Publish pipeline**: dequeue → ignore unless the version is Queued/Building → Building → freeze drafts into `snapshots/{versionId}` → assemble the tree from the snapshot + the frozen manifest → validate structure → security-scan the composed tree (any Block finding → version `Rejected`, nothing uploaded) → deterministic zip + sha256 → upload `public/z/...` → pinned `public/m/{name}/{semanticVersion}/marketplace.json` → persist Published + LatestVersionId → regenerate the root marketplace.json. Poison queue after `maxDequeueCount` (5) total attempts, including the first.
