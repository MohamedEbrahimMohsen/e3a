...
1. **Install command block** — dark mono block with two lines (`/plugin marketplace add https://e3a.ai/marketplace.json`, `/plugin install e3a-mmohsen@e3a`), each with its own copy button; compact variant for cards.
...
3. **Version history row** — version badge, published date, size, sha256 (truncated, mono), and a "pin this version" copy command that expands the pinned-marketplace command (`/plugin marketplace add https://e3a.ai/m/{plugin}/{version}/marketplace.json`).
...
9. **Team composer** — left: team form (name, description, tags); right: member picker (search published engineers, add with version pin dropdown), ordered member list with keyboard-accessible move up/down controls (drag-and-drop deferred), structure preview; same sticky footer.
