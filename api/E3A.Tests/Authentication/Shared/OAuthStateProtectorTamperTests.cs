...
public sealed class OAuthStateProtectorTamperTests
{
    ...
    {
        var result = _sut.Validate(state, Nonce);
    ...
    {
        var segments = _sut.Create().Value.Split('.');

        var result = _sut.Validate($"{segments[0]}.later.{segments[2]}", Nonce);
    ...
    public void Validate_ShouldReturnInvalid_WhenNonceIsTampered()
    {
        ...
        var result = _sut.Validate($"tampered.{segments[1]}.{segments[2]}", Nonce);
    ...
    public void Validate_ShouldReturnInvalid_WhenExpiryIsExtendedWithoutResigning()
    {
        ...
        var result = _sut.Validate($"{segments[0]}.{extendedExpiry}.{segments[2]}", Nonce);
    ...
    public void Validate_ShouldReturnInvalid_WhenExpiryIsMovedIntoThePastWithoutResigning()
    {
        ...
        var result = _sut.Validate($"{segments[0]}.{pastExpiry}.{segments[2]}", Nonce);
    ...
    public void Validate_ShouldReturnInvalid_WhenSignatureIsTampered()
    {
        ...
        var result = _sut.Validate($"{segments[0]}.{segments[1]}.{new string('A', segments[2].Length)}", Nonce);
    ...
    public void Validate_ShouldReturnInvalid_WhenSignatureIsTruncated()
    {
        ...
        var result = _sut.Validate($"{segments[0]}.{segments[1]}.{segments[2][..(segments[2].Length / 2)]}", Nonce);
    ...
    public void Validate_ShouldReturnInvalid_WhenStateWasSignedWithADifferentKey()
    {
        ...
        var result = _sut.Validate(foreignState.Value, foreignState.Nonce);
