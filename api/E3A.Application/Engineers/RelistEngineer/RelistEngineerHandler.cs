...
public sealed class RelistEngineerHandler(IEngineerRepository engineerRepository, IItemVersionRepository itemVersionRepository, ICurrentUserService currentUserService, ISender sender) : IRequestHandler<RelistEngineerCommand, EngineerResult>
{
    public async Task<EngineerResult> Handle(RelistEngineerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (engineer.Status != EngineerStatus.Unlisted)
        {
            throw new BusinessRuleViolationCoreException(ErrorCodes.EngineerNotUnlisted);
        }
