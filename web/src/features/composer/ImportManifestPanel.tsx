...
const sectionHeaderStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600, width: '100%', textAlign: 'left', background: 'none', border: 'none', fontFamily: 'inherit', color: 'inherit' };
const inlineToggleStyle: React.CSSProperties = { padding: 0, textAlign: 'left', background: 'none', border: 'none', fontFamily: 'inherit', fontWeight: 400 };
...
export function ImportManifestPanel({ manifest, onReplace }: { manifest: ImportManifest; onReplace: () => void }) {
  ...
  const section = (key: string, label: string, color: string, marker: string, count: number, rows: React.ReactNode) => (
    <div className="card" style={{ overflow: 'hidden' }}>
      <button type="button" onClick={() => toggle(key)} aria-expanded={expanded[key] === true} className="hover-row" style={sectionHeaderStyle}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {manifest.claudeMdSnippet !== null && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button type="button" onClick={() => toggle('snippet')} aria-expanded={expanded.snippet === true} className="link-quiet" style={{ ...inlineToggleStyle, fontSize: 12.5, color: 'var(--text-secondary)' }}>view snippet {expanded.snippet ? '▾' : '▸'}</button>
      ...
      {manifest.strippedPaths.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <button type="button" onClick={() => toggle('stripped')} aria-expanded={expanded.stripped === true} className="link-quiet" style={{ ...inlineToggleStyle, fontSize: 12, color: 'var(--text-muted)' }}>{manifest.strippedPaths.length} local files stripped {expanded.stripped ? '▾' : '▸'}</button>
