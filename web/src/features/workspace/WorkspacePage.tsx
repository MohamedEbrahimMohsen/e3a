import { useEffect, useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { emojiFor } from '../../lib/api';
import { listMyEngineers, listMyTeams } from '../../lib/workspaceApi';
import { formatInstallCount, formatUpdated, toWorkspaceRows, type WorkspaceRow } from './workspaceRows';

const gridColumns = '2.4fr 0.9fr 1.1fr 0.9fr 1.1fr 1.6fr';

function statusChipStyle(status: string): React.CSSProperties {
  if (status === 'Published') {
    return { color: 'var(--success)', background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.3)' };
  }
  if (status === 'Unlisted') {
    return { color: 'var(--warning)', background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.3)' };
  }
  return { color: 'var(--text-secondary)', background: 'var(--surface-elevated)', border: '1px solid var(--border)' };
}

export function WorkspacePage() {
  const navigate = useNavigate();
  const [rows, setRows] = useState<WorkspaceRow[]>([]);
  const [status, setStatus] = useState<'loading' | 'ready' | 'failed'>('loading');
  const [reloadToken, setReloadToken] = useState(0);

  useEffect(() => {
    let cancelled = false;
    Promise.all([listMyEngineers(), listMyTeams()])
      .then(([engineers, teams]) => { if (!cancelled) { setRows(toWorkspaceRows(engineers, teams)); setStatus('ready'); } })
      .catch(() => { if (!cancelled) { setStatus('failed'); } });
    return () => { cancelled = true; };
  }, [reloadToken]);

  const newItemButtons = (
    <div style={{ display: 'flex', gap: 12 }}>
      <button type="button" onClick={() => navigate('/workspace/new-engineer')} className="btn-primary">+ New Engineer</button>
      <button type="button" onClick={() => navigate('/workspace/new-team')} className="btn-secondary">+ New Team</button>
    </div>
  );

  return (
    <div className="page fade-in" style={{ gap: 28 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 32 }}>
        <h1 style={{ fontSize: 26, fontWeight: 700 }}>My workspace</h1>
        {newItemButtons}
      </div>
      {status === 'loading' && <div style={{ padding: '72px 40px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13.5 }}>Loading…</div>}
      {status === 'failed' && (
        <div className="fade-in" style={{ padding: '72px 40px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center' }}>
          <span style={{ fontSize: 16, fontWeight: 700 }}>Could not load your workspace</span>
          <span style={{ fontSize: 13.5, color: 'var(--text-secondary)' }}>The API is unreachable. Check that it is running, then retry.</span>
          <button type="button" onClick={() => { setStatus('loading'); setReloadToken(token => token + 1); }} className="btn-secondary" style={{ padding: '8px 18px', fontSize: 13, marginTop: 6 }}>Retry</button>
        </div>
      )}
      {status === 'ready' && rows.length === 0 && (
        <div className="fade-in" style={{ padding: '72px 40px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center' }}>
          <span style={{ fontSize: 16, fontWeight: 700 }}>Nothing here yet</span>
          <span style={{ fontSize: 13.5, color: 'var(--text-secondary)' }}>Compose your first engineer — upload your .claude folder and publish it.</span>
          {newItemButtons}
        </div>
      )}
      {status === 'ready' && rows.length > 0 && (
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: gridColumns, gap: 16, padding: '12px 24px', fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', borderBottom: '1px solid var(--border)' }}>
            <span>Name</span><span>Type</span><span>Status</span><span>Installs</span><span>Updated</span><span style={{ textAlign: 'right' }}>Actions</span>
          </div>
          {rows.map(row => (
            <div key={row.itemId} className="hover-row" style={{ display: 'grid', gridTemplateColumns: gridColumns, gap: 16, padding: '16px 24px', fontSize: 13, alignItems: 'center', borderBottom: '1px solid var(--surface-elevated)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 16 }}>{emojiFor(row.slug)}</span>
                <span className="mono" style={{ fontSize: 13 }}>{row.slug}</span>
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{row.displayName}</span>
              </span>
              <span style={{ color: 'var(--text-secondary)' }}>{row.type}</span>
              <span><span style={{ fontSize: 11.5, fontWeight: 600, borderRadius: 999, padding: '3px 11px', ...statusChipStyle(row.status) }}>{row.status}</span></span>
              <span className="mono" style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{formatInstallCount(row.installCount)}</span>
              <span style={{ color: 'var(--text-muted)', fontSize: 12.5 }}>{formatUpdated(row.updatedAt)}</span>
              <span style={{ display: 'flex', gap: 14, justifyContent: 'flex-end', fontSize: 12.5 }}>
                <Link to={row.editPath} className="link-violet">Edit</Link>
                <Link to={row.actionPath} className="link-accent-hover" style={{ color: 'var(--accent)', cursor: 'pointer' }}>{row.actionLabel}</Link>
                <Link to={row.viewPath} className="link-quiet" style={{ color: 'var(--text-secondary)', cursor: 'pointer' }}>View</Link>
              </span>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
